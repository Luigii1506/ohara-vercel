...
import SingleSelect, {
  type Option as SingleSelectOption,
...
export default function ApproveMissingSetPage() {
  ...
  const [selectedReassignCards, setSelectedReassignCards] = useState<number[]>(
    []
  );
  const [eventCardImageUrl, setEventCardImageUrl] = useState<string | null>(
    null
  );
  ...
  const handleClassificationChange = (
    ...
    if (eventCardImageUrl === url && value !== "CARD" && value !== "DON") {
      setEventCardImageUrl(null);
    }
  };
  ...
  const handleSetCodeChange = async (imageUrl: string, setCode: string) => {
    ...
    try {
      // Para cartas Promo, convertir "PROMO" a cadena vacía (el setCode real en la BD)
  ...
  const buildApprovalPayload = (): ApprovalRequestPayload => {
    ...
    let eventCardPayload:
      | ApprovalRequestPayload["eventCardPayload"]
      | undefined;
  ...
  const submitApproval = async (payload: ApprovalRequestPayload) => {
    ...
    if (payload.action === "linkExisting") {
      ...
      showSuccessToast(
        `Set "${result.setTitle}" creado y ${
          result.reassignedCards ?? 0
        } cartas reasignadas.`
      );
    } else {
      showSuccessToast(
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-3 text-sm text-muted-foreground">
                ...
                <span>•</span>
                <span>
                  {missingSet.images.length} imagen(es) por clasificar
                </span>
            ...
            {approvalMode === "linkExisting" && (
              <Card>
                <CardHeader>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Verifica que las cartas coincidan con las imágenes
                    detectadas antes de aprobar.
                  </p>
                </CardHeader>
                <CardContent>
                  {!selectedExistingSet ? (
                ...
                >
                  <div className="flex items-start space-x-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Genera un nuevo set usando las imágenes clasificadas y
                        crea cartas alternas.
                      </p>
                    </div>
                  </div>
                  <div className="flex items-start space-x-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Vincula este missing set a un set que ya existe en la
                        base de datos sin crear cartas nuevas.
                      </p>
                    </div>
                  </div>
                  <div className="flex items-start space-x-2">
                  ...
                  <div className="flex items-start space-x-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Crea un set nuevo y mueve cartas existentes al set
                        correcto sin duplicarlas.
                      </p>
                    </div>
                  </div>
                </RadioGroup>
                      ...
                      <div className="rounded-lg border p-3 space-y-2 text-sm">
                        ...
                        {!loadingExistingSetCards &&
                          !selectedExistingSetCards.length && (
                            <p className="text-xs text-muted-foreground">
                              Selecciona el set para ver sus cartas y confirmar
                              que coinciden con las imágenes detectadas.
                            </p>
                          )}
                      </div>
                    )}
                ...
                {approvalMode === "eventCardOnly" && (
                  <div className="rounded-lg border p-3 space-y-4 text-sm">
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Usa esta opción cuando el missing set en realidad era
                        una sola carta. Busca el código correspondiente, elige
                        la variante correcta o crea una alterna con la imagen
                        seleccionada.
                      </p>
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      {eventCardImageUrl ? (
                        ...
                        <p className="text-xs text-muted-foreground">
                          Selecciona una imagen en la galería y marca &quot;Usar
                          para carta del evento&quot; para continuar.
                        </p>
                      )}
                    </div>
                    ...
                    <form
                      className="space-y-2"
                      onSubmit={handleEventCardSearch}
                    >
                      <label className="text-xs font-semibold text-muted-foreground">
                        Código de la carta
                      </label>
                      <div className="flex gap-2">
                        ...
                        <Button
                          type="submit"
                          disabled={eventCardOptionsLoading}
                        >
                      ...
                      <p className="text-[11px] text-muted-foreground">
                        Ingresa el código exacto tal como aparece en la carta
                        para cargar todas sus variantes registradas.
                      </p>
                    </form>
                      ...
                      {eventCardOptionsLoading ? (
                        ...
                        <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                          {eventCardOptions.map((card) => {
                            const isSelected =
                              card.id === eventCardSelectedCardId;
                            ...
                            return (
                              <button
                                ...
                                key={card.id}
                                onClick={() =>
                                  setEventCardSelectedCardId(card.id)
                                }
                        ...
                        <p className="text-xs text-muted-foreground">
                          No encontramos cartas con ese código. Verifica que
                          esté escrito correctamente o intenta con otra
                          variante.
                        </p>
                      ) : (
                        <p className="text-xs text-muted-foreground">
                    ...
                    {eventCardOptions.length > 0 && (
                      <div className="space-y-2">
                        ...
                        <RadioGroup
                          ...
                          onValueChange={(value) =>
                            setEventCardModeOption(
                              value as "existing" | "alternate"
                            )
                          }
                          className="space-y-1"
                        >
                    ...
                    <div className="p-3 bg-destructive/10 border border-destructive/20 rounded-lg">
                      <div className="flex gap-2 items-start">
                        ...
                        <p className="text-xs text-destructive">
                          Debes clasificar todas las imágenes antes de aprobar
                          el set.
                        </p>
                      </div>
                    </div>
