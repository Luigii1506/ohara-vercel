  ...
  return (
    <div className="min-h-screen bg-[#f7f2e6]">
      <div className="mx-auto flex w-full max-w-[1600px] flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm text-slate-600">
                Explora un código completo por región, detecta repetidas contra
                exclusivas y corrige `region` o `isRegionalExclusive` de forma
                visual.
              </p>
            </div>
          ...
          >
            <div className="relative w-full max-w-xl">
              ...
              <input
                ...
                className="h-12 w-full rounded-2xl border border-slate-200 bg-slate-50 pl-10 pr-4 text-sm text-slate-900 outline-none transition focus:border-slate-400 focus:bg-white"
              />
            </div>
        ...
        {cards.length > 1 ? (
          <Card className="rounded-[28px] border-slate-200 bg-white p-4 shadow-sm sm:p-5">
            <div className="mb-3">
              <h2 className="text-sm font-semibold text-slate-900">
                Punto de partida
              </h2>
              <p className="text-xs text-slate-500">
                Elige cualquier versión de este código para abrir la misma matriz.
              </p>
            </div>
              ...
              {cards.map((card) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className={`mt-2 text-sm ${active ? "text-white/85" : "text-slate-600"}`}>
                      {card.alternateArt?.trim() || card.name}
                    </p>
                    <p className={`mt-1 text-xs ${active ? "text-white/65" : "text-slate-500"}`}>
        ...
        {loading ? (
          ...
          <RegionVariantMatrix
            ...
            title="Vista completa por región"
