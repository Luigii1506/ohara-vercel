export const dynamic = 'force-dynamic';

// API para gestión de inventario del vendedor - POST para crear productos
// Fecha de modificación: 2025-01-19

import { NextRequest, NextResponse } from "next/server";
import { getServerSession } from "next-auth";
import { PrismaClient } from "@prisma/client";

const prisma = new PrismaClient();

export async function POST(request: NextRequest) {
  try {
    const session = await getServerSession();

    if (!session?.user?.email) {
      return NextResponse.json(
        { success: false, error: "No autorizado" },
        { status: 401 }
      );
    }

    const user = await prisma.user.findUnique({
      where: { email: session.user.email },
    });

    if (!user || (user.role !== "SELLER" && user.role !== "ADMIN")) {
      return NextResponse.json(
        { success: false, error: "Sin permisos de vendedor" },
        { status: 403 }
      );
    }

    const body = await request.json();
    const {
      cardId,
      price,
      stock,
      conditionForSale,
      sku,
      lowStockThreshold = 5,
      isDraft = true, // Por defecto como borrador en inventario
      isActive = false, // Por defecto inactivo hasta que se liste
    } = body;

    // Validaciones
    if (!cardId || !price || !stock || !conditionForSale) {
      return NextResponse.json(
        { success: false, error: "Faltan campos obligatorios" },
        { status: 400 }
      );
    }

    if (price <= 0 || stock < 0) {
      return NextResponse.json(
        { success: false, error: "Precio y stock deben ser positivos" },
        { status: 400 }
      );
    }

    // Verificar que la carta existe
    const card = await prisma.card.findUnique({
      where: { id: parseInt(cardId) },
    });

    if (!card) {
      return NextResponse.json(
        { success: false, error: "Carta no encontrada" },
        { status: 404 }
      );
    }

    // Verificar que no existe un listado activo de esta carta por este vendedor con la misma condición
    const existingListing = await prisma.cardListing.findFirst({
      where: {
        cardId: parseInt(cardId),
        sellerId: user.id,
        conditionForSale,
        isActive: true,
      },
    });

    if (existingListing) {
      return NextResponse.json(
        {
          success: false,
          error: "Ya tienes un listado activo de esta carta en esta condición",
        },
        { status: 409 }
      );
    }

    // Crear el listado en inventario
    const newListing = await prisma.cardListing.create({
      data: {
        cardId: parseInt(cardId),
        sellerId: user.id,
        price: Math.round(parseFloat(price) * 100), // Convertir a centavos
        stock: parseInt(stock),
        conditionForSale,
        sku: sku || `${card.code}-${conditionForSale}-${user.id}-${Date.now()}`,
        lowStockThreshold: parseInt(lowStockThreshold),
        isDraft,
        isActive,
      },
      include: {
        card: true,
        seller: {
          select: {
            id: true,
            name: true,
            email: true,
          },
        },
      },
    });

    return NextResponse.json({
      success: true,
      data: newListing,
      message: "Producto agregado al inventario exitosamente",
    });
  } catch (error) {
    console.error("Error creando producto en inventario:", error);
    return NextResponse.json(
      { success: false, error: "Error interno del servidor" },
      { status: 500 }
    );
  }
}

// GET - Obtener inventario del vendedor
export async function GET(request: NextRequest) {
  try {
    const session = await getServerSession();

    if (!session?.user?.email) {
      return NextResponse.json(
        { success: false, error: "No autorizado" },
        { status: 401 }
      );
    }

    const user = await prisma.user.findUnique({
      where: { email: session.user.email },
    });

    if (!user || (user.role !== "SELLER" && user.role !== "ADMIN")) {
      return NextResponse.json(
        { success: false, error: "Sin permisos de vendedor" },
        { status: 403 }
      );
    }

    const { searchParams } = new URL(request.url);
    const page = parseInt(searchParams.get("page") || "1");
    const limit = parseInt(searchParams.get("limit") || "20");
    const search = searchParams.get("search");
    const isListed = searchParams.get("isListed");
    const lowStock = searchParams.get("lowStock");

    const skip = (page - 1) * limit;

    // Construir condiciones WHERE
    const whereConditions: any = {
      sellerId: user.id, // Solo inventario del vendedor actual
    };

    // Filtro de búsqueda
    if (search) {
      whereConditions.OR = [
        {
          card: {
            name: {
              contains: search,
              mode: "insensitive",
            },
          },
        },
        {
          card: {
            code: {
              contains: search,
              mode: "insensitive",
            },
          },
        },
        {
          sku: {
            contains: search,
            mode: "insensitive",
          },
        },
      ];
    }

    // Filtro por estado de listado
    if (isListed !== null && isListed !== undefined) {
      if (isListed === "true") {
        whereConditions.isActive = true;
        whereConditions.isDraft = false;
      } else if (isListed === "false") {
        whereConditions.OR = [{ isActive: false }, { isDraft: true }];
      }
    }

    // Filtro de stock bajo
    if (lowStock === "true") {
      whereConditions.stock = {
        gt: 0,
        lte: 5, // Stock bajo definido como <= 5
      };
    }

    // Ejecutar consultas
    const [items, total] = await Promise.all([
      prisma.cardListing.findMany({
        where: whereConditions,
        include: {
          card: {
            select: {
              id: true,
              name: true,
              code: true,
              src: true,
              setCode: true,
              category: true,
              rarity: true,
            },
          },
          seller: {
            select: {
              id: true,
              name: true,
              email: true,
            },
          },
        },
        orderBy: {
          createdAt: "desc",
        },
        skip,
        take: limit,
      }),
      prisma.cardListing.count({
        where: whereConditions,
      }),
    ]);

    // Adaptar formato para la página de inventario
    const adaptedItems = items.map((item) => ({
      id: item.id,
      cardId: item.cardId,
      sku: item.sku || "",
      card: {
        name: item.card.name,
        code: item.card.code,
        src: item.card.src,
        setCode: item.card.setCode,
      },
      condition: item.conditionForSale,
      language: "ES", // TODO: Agregar al modelo si es necesario
      currentStock: item.stock,
      reservedStock: 0, // TODO: Implementar reservas cuando sea necesario
      availableStock: item.stock,
      minStockLevel: item.lowStockThreshold,
      maxStockLevel: item.lowStockThreshold * 4, // Estimación
      reorderPoint: item.lowStockThreshold,
      costPrice: (Number(item.price) / 100) * 0.7, // Estimación: 70% del precio de venta
      listPrice: Number(item.price) / 100, // Convertir de centavos a dólares
      margin: 30, // Estimación: 30% de margen
      lastRestocked: item.createdAt.toISOString().split("T")[0],
      supplier: "", // TODO: Implementar cuando sea necesario
      location: "", // TODO: Implementar cuando sea necesario
      isListed: item.isActive && !item.isDraft,
      status: item.isActive && !item.isDraft ? "active" : "discontinued",
      movementHistory: [], // TODO: Implementar historial cuando sea necesario
    }));

    const totalPages = Math.ceil(total / limit);

    return NextResponse.json({
      success: true,
      data: {
        items: adaptedItems,
        pagination: {
          page,
          limit,
          total,
          last_page: totalPages,
          has_next: page < totalPages,
          has_prev: page > 1,
        },
      },
    });
  } catch (error) {
    console.error("Error obteniendo inventario:", error);
    return NextResponse.json(
      { success: false, error: "Error interno del servidor" },
      { status: 500 }
    );
  }
}
