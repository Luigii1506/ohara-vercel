...
const MyDeck = () => {
  const { id } = useParams(); // URL único del deck original
  ...
  const handleEdit = () => {
    // Usar el ID numérico del deck cargado en lugar del parámetro de la URL
    const deckIdToEdit = deckData?.id || id;
    router.push(`/decks/edit/${deckIdToEdit}`);
  };

  // Función para hacer fork del deck
  const handleFork = async () => {
    if (!session?.user) {
      alert("Debes iniciar sesión para copiar un deck");
      return;
    }
    ...
    try {
      // Usar el ID numérico del deck cargado en lugar del parámetro de la URL
      const deckIdToFork = deckData?.id || id;
      const response = await fetch(`/api/decks/${deckIdToFork}/fork`, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        throw new Error("Error al copiar el deck");
      }
      ...
      // Redirigir al deck copiado
      router.push(`/decks/${data.uniqueUrl}`);
    } catch (error) {
      console.error("Error al copiar deck:", error);
      alert("Error al copiar el deck. Inténtalo de nuevo.");
    } finally {
      setIsForking(false);
    }
  };
  ...
  // Fetch del deck original mediante su id o uniqueUrl
  useEffect(() => {
    const fetchDeck = async () => {
      try {
        // Determinar si el parámetro es un ID numérico o un uniqueUrl
        ...
        // Verificar si el usuario actual es el propietario del deck
        const currentUserId = session?.user?.id;
        setIsOwner(
          Boolean(currentUserId && data.userId === parseInt(currentUserId))
        );
      } catch (error) {
        console.error(error);
      }
  ...
  return (
    <div className="flex flex-col flex-1 bg-[#f2eede] w-full lg:flex-row">
      <div className="flex flex-col flex-shrink-0 h-full w-full px-4 py-4 gap-4 bg-white">
          ...
          <div className="flex justify-between gap-2 max-w-none w-full sm:max-w-[500px] sm:w-screen">
            {isOwner ? (
              ...
              <Button
                ...
                className="flex-1 gap-2 bg-green-500 hover:bg-green-600 text-white py-6 transition-all hover:shadow-lg hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <Copy className="w-5 h-5" />
                <span className="text-lg font-medium">
                  {isForking ? "Copiando..." : "Copiar"}
                </span>
              </Button>
            )}
                    ...
                    <div className="flex w-full flex-col space-y-2">
                      <div className="flex items-center space-x-2">
                        <Input
                          id="deck-url"
                          type="text"
                          value={"https://oharatcg.com/decks/" + id}
                          readOnly
                        ...
                        <Button
                          ...
                          onClick={() =>
                            handleCopyUrl("https://oharatcg.com/decks/" + id)
                          }
                        >
                          {copiedUrl ? (
