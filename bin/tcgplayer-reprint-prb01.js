...
const BASE_URL =
  "https://www.tcgplayer.com/search/one-piece-card-game/premium-booster-the-best";
const QUERY = "reprint";
const USER_AGENT =
...
async function scrapePage(context, pageNumber = 1) {
  const url = `${BASE_URL}?productLineName=one-piece-card-game&q=${encodeURIComponent(
    QUERY
  )}&view=grid&ProductTypeName=Cards&page=${pageNumber}&setName=premium-booster-the-best`;
  console.log(`[navigate][page ${pageNumber}] ${url}`);

  const page = await context.newPage();
  await page.goto(url, { waitUntil: "domcontentloaded", timeout: 60000 });
  const currentUrl = page.url();
  if (!currentUrl.includes("premium-booster-the-best")) {
    console.log(
...
async function scrapeReprintCards({ maxPages = 3 } = {}) {
