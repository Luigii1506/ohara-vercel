...
const CollectionBinderView = ({ shareToken }: { shareToken?: string }) => {
  ...
  const publicCollectionIdParam = searchParams.get("collectionId");
  const publicCollectionId = publicCollectionIdParam
    ? Number.parseInt(publicCollectionIdParam, 10)
    : null;
  const publicToken = shareToken || searchParams.get("token");
  const isPublicView = Boolean(publicToken || Number.isFinite(publicCollectionId ?? NaN));
  ...
  const fetchCollection = async () => {
    try {
      const endpoint = isPublicView
        ? `/api/collection/public?${publicToken ? `token=${publicToken}` : `collectionId=${publicCollectionId}`}&includeSlots=1&limit=0`
        : `/api/collection?limit=0&includeSlots=1`;
      ...
      toast.error(t("collection.binder.collectionLoadError"));
    } finally {
      setLoading(false);
    }
  };
  ...
  }, [status, rows, cols, isPublicView, publicCollectionId, publicToken]);
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center p-4">
        <div className="text-center max-w-md">
          ...
          <h1 className="text-3xl md:text-4xl font-bold text-white mb-4">
            {t("collection.binder.loginTitle")}
          </h1>
          <p className="text-slate-300 mb-8">
            {t("collection.binder.loginSubtitle")}
          </p>
          <Button
            size="lg"
          ...
          >
            ...
            {t("collection.binder.loginButton")}
          </Button>
        </div>
      </div>
    );
      ...
      <div className="h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100">
        <div className="text-center bg-white p-8 rounded-xl shadow-lg">
          <h3 className="text-xl font-semibold text-slate-900 mb-2">
            {t("collection.binder.emptyTitle")}
          </h3>
          <p className="text-slate-600 mb-4">
            {t("collection.binder.emptySubtitle")}
          </p>
          <Button
          ...
          >
            ...
            {t("collection.binder.backToCollection")}
          </Button>
        </div>
      </div>
    );
      ...
      >
        <div className="h-full overflow-hidden bg-gradient-to-br from-slate-50 to-slate-100 flex flex-col relative w-full">
          ...
          <div className="md:hidden fixed top-0 left-0 right-0 z-30 px-3 pt-[calc(env(safe-area-inset-top)+10px)] pb-2">
            <div
              className={`grid items-center gap-2 rounded-2xl bg-white/90 px-3 py-2 shadow-lg backdrop-blur ${
                isPublicView ? "grid-cols-1" : "grid-cols-[auto,1fr]"
              }`}
            >
              {!isPublicView && (
                <button
                ...
                >
                  ...
                  {t("collection.binder.back")}
                </button>
              )}
              <div className="min-w-0 text-center text-xs font-semibold text-slate-700 truncate">
                {dims.showSinglePage &&
                  (safePage === 0
                    ? t("collection.binder.cover")
                    : t("collection.binder.pageOf", {
                        page: safePage,
                        total: totalPages,
                      }))}
              </div>
          ...
          {dims.showSinglePage && (
            <div className="absolute top-4 left-1/2 -translate-x-1/2 z-10 hidden md:block">
              <div className="bg-black/80 backdrop-blur-sm text-white px-4 py-2 rounded-full text-sm font-medium shadow-lg">
                {safePage === 0
                  ? t("collection.binder.insideCover")
                  : t("collection.binder.pageOf", {
                      page: safePage,
                      total: totalPages,
                    })}
          ...
          <div className="flex-1 flex items-center justify-center p-1 pt-16 sm:p-4 relative min-h-0 md:pt-4">
            <BookFlipContainer
              name={t("collection.binder.collectionName")}
              color="#1b2416"
              dimensions={dims}
        ...
        >
          ...
          >
            <div className="text-white text-xl lg:text-2xl font-[400] text-center py-2 px-5">
              {t("collection.binder.tapToClose")}
            </div>
...
export default CollectionBinderView;
