...
import TenCostIcon from "../Icons/TenCostIcon";
...
export default function GroupedCardPreview({
  group,
  onQuantityChange,
  onSwipeLeft,
  index,
    ...
    >
      ...
      >
        <div className="flex items-center gap-5">
          ...
          <span className={`${oswald.className} whitespace-nowrap`}>
            {code}
          </span>
          <span className="font-medium line-clamp-1">{name}</span>
        </div>
        <span className="pl-2">{rarityFormatter(rarity)}</span>
      </div>
      ...
      {group.map((card, index) => (
        ...
        >
          ...
          <div className="flex items-center gap-4 flex-1 justify-between">
            ...
            <button
              onClick={() => onQuantityChange(card.cardId, card.quantity + 1)}
              className="w-8 h-8 flex items-center justify-center rounded-full border border-zinc-200 hover:bg-zinc-100 transition-colors"
            >
