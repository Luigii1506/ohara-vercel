...
/**
 * Script para crear cartas "3rd Anniversary" en la base de datos.
 * - Hace scraping de TCGplayer para el set "Carrying On His Will 3rd Anniversary Tournament Pack".
 * - Ignora cartas cuyo título contenga "(3rd Anniversary Tournament 3 Brothers Pack)".
 * - Busca la carta base (isFirstEdition: true) por código y crea una alterna con alternateArt="3rd Anniversary".
 * - Sube las variantes de imagen a R2 y relaciona la carta con el set objetivo.
 *
 * Uso:
 *   npx ts-node scripts/process-anniversary-set-3rd.ts --maxPages=4
 */
...
const TARGET_SET_NAME = TARGET_SLUG;
const TARGET_SET_TITLE =
  "Carrying On His Will 3rd Anniversary Tournament Pack";
const ALTERNATE_ART_LABEL = "3rd Anniversary";
const DEFAULT_MAX_PAGES = 4;
const TITLE_EXCLUSION = "(3rd Anniversary Tournament 3 Brothers Pack)";
    ...
    (titles: Element[]) => {
      ...
      return titles
        .map((titleEl) => {
          ...
          if (title.includes(TITLE_EXCLUSION)) {
            return null;
          }
...
async function main() {
  console.log(`Processing 3rd Anniversary set pages=${cli.maxPages}`);

  const cards = await scrapeAllCards();
  console.log(`Scraped ${cards.length} unique cards from TCGplayer.`);
