...
const parseArgs = (): ScriptOptions => {
  ...
  for (const arg of args) {
    if (arg.startsWith("--limit=")) {
      ...
      options.offerTypeFilter = value
        .split(",")
        .map((item) => item.trim())
        .filter(Boolean);
    } else if (arg.startsWith("--offer-type-pattern=")) {
      const value = arg.split("=")[1] ?? "";
      options.offerTypePattern = value.trim() || null;
    ...
    } else if (arg === "--promote-alternate-base") {
      options.promoteAlternateToBase = true;
...
const splitBySlash = (value: string | null | undefined): string[] => {
  if (!value) return [];
  return value
    .split(/[/／]/)
    .map((item) => item.trim())
...
const parseCardInfo = (
  ...
  if (info.cardOfferType) {
    console.log(`[offer][detail] ${baseCode} -> ${info.cardOfferType}`);
  }
...
const resolveOfferTypeList = (
  ...
  if (!options.offerTypeOrderProvided) {
    ...
    list = [...list].sort((a, b) => {
      ...
      const aNum = parseNumericSuffix(aCode) ?? parseNumericSuffix(a.offerType) ?? null;
      const bNum = parseNumericSuffix(bCode) ?? parseNumericSuffix(b.offerType) ?? null;
      if (aNum !== null && bNum !== null && aNum !== bNum) return aNum - bNum;
      const aLabel = aCode || a.offerType;
      const bLabel = bCode || b.offerType;
      return aLabel.localeCompare(bLabel, "en", { numeric: true });
    });
  } else if (options.offerTypeOrder === "asc" || options.offerTypeOrder === "desc") {
...
const fetchOfferTypeCards = async (
  ...
  while (page <= totalPage) {
    console.log(`[page] Fetching list page ${page}/${totalPage}`);
    const listResponse = await fetchCardListPage(offerType, page, pageSize);
    totalPage = listResponse.page?.totalPage ?? 1;
    const listCount = listResponse.page?.list?.length ?? 0;
    const totalCount = listResponse.page?.totalCount ?? 0;
    console.log(
      `[page] Loaded ${listCount} ids (page ${page}/${totalPage}, total ${totalCount})`
    );
    const listItems = listResponse.page?.list ?? [];
    for (let i = 0; i < listItems.length; i += 5) {
      const batch = listItems.slice(i, i + 5);
      console.log(`[batch] Fetching ${batch.length} card infos`);
...
const cleanupDuplicateSources = async (
  ...
  for (let i = 0; i < groupEntries.length; i += 1) {
    const [imageUrl, entries] = groupEntries[i];
...
const main = async () => {
  ...
  if (!offerTypeList.length) {
    console.log("No offer types matched the provided filters.");
    return;
  }

  console.log(
    `[start] OfferTypes=${offerTypeList.length} Region=${options.region} Lang=${options.language}`
  );

  let processed = 0;
  const unresolvedAlternates: ScrapedCard[] = [];

  for (const entry of offerTypeList) {
    ...
    const defaultSetCode = entry.setCode ?? normalizedCards[0]?.setCode ?? null;
    ...
    for (const card of bases) {
      ...
      const originTitle =
        card.originTitle?.trim() || entry.offerType?.trim() || null;
    ...
    const promoteAlternate = async (
      ...
      console.log(
        `[promote][alternate-base] ${card.baseCode} from ${card.id}`
      );
      ...
      if (createdId) {
        baseCardIdByCode.set(card.baseCode, createdId);
      }
      return createdId;
    };
  ...
  if (options.cleanupDuplicates && !options.dryRun) {
    await cleanupDuplicateSources("CN", options.region, options.dryRun);
  } else if (options.cleanupDuplicates && options.dryRun) {
    await cleanupDuplicateSources("CN", options.region, options.dryRun);
  }
  ...
  if (missingImages.length) {
    ...
    if (missingImages.length > 50) {
      console.log(
        `[summary] ${missingImages.length - 50} more missing images not shown.`
      );
    }
    await fs.writeFile(
      missingFile,
    ...
    console.log(`[summary] Saved missing list to ${missingFile}`);
  }

  if (networkErrors.length && !missingImages.length) {
    console.log(
      `\n[summary] Network errors (${networkErrors.length}):`,
      networkErrors.slice(0, 50)
    );
    if (networkErrors.length > 50) {
      console.log(
        `[summary] ${networkErrors.length - 50} more network errors not shown.`
      );
    }
    await fs.writeFile(
      missingFile,
  ...
  if (missingUsBaseCards.length) {
    const outPath = path.join(process.cwd(), missingUsBasePath);
    await fs.writeFile(
      outPath,
      JSON.stringify({ missingUsBaseCards }, null, 2)
    );
    console.log(
      `[summary] Saved missing US base cards to ${outPath} (${missingUsBaseCards.length})`
    );
  }
