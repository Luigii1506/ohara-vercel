...
const BASE_URL = "https://fr.onepiece-cardgame.com";
...
const parseCardModal = (
  ...
  const life =
    normalizedCostLabel.includes("VIE") && costNumber
      ? `${costNumber} Life`
      : null;
  const cost =
...
const main = async () => {
  ...
  if (options.cleanupDuplicates) {
    await cleanupDuplicateSources("FR", options.region, options.dryRun);
  }
};
