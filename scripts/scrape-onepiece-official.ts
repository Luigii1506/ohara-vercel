...
// Registro de regiones oficiales (plataforma Bandai, misma estructura HTML).
const REGION_CONFIG: Record<
  string,
  { baseUrl: string; region: string; language: string }
> = {
  EN: { baseUrl: "https://en.onepiece-cardgame.com", region: "EN", language: "en" },
  "ASIA-EN": {
    baseUrl: "https://asia-en.onepiece-cardgame.com",
    region: "ASIA-EN",
    language: "en",
  },
  JP: { baseUrl: "https://www.onepiece-cardgame.com", region: "JP", language: "ja" },
  FR: { baseUrl: "https://fr.onepiece-cardgame.com", region: "FR", language: "fr" },
};

// Se resuelven en main() según --region.
let BASE_URL = "https://en.onepiece-cardgame.com";
let SOURCE_TAG = "EN";
...
const SERIES_LIST: SeriesEntry[] = [
  { series: "622109", setCode: "OP-09" },
  { series: "622110", setCode: "OP-10" },
  { series: "622111", setCode: "OP-11" },
  { series: "622112", setCode: "OP-12" },
  { series: "622113", setCode: "OP-13" },
  { series: "622202", setCode: "EB-02" },
  { series: "622114", setCode: "OP14-EB04" },

  { series: "622301", setCode: "PRB-01" },
  { series: "622302", setCode: "PRB-02" },

  { series: "622015", setCode: "ST-15" },
  { series: "622016", setCode: "ST-16" },
  { series: "622017", setCode: "ST-17" },
  { series: "622018", setCode: "ST-18" },
  { series: "622019", setCode: "ST-19" },
  { series: "622020", setCode: "ST-20" },
  { series: "622021", setCode: "ST-21" },
  { series: "622022", setCode: "ST-22" },
  { series: "622023", setCode: "ST-23" },
  { series: "622024", setCode: "ST-24" },
  { series: "622025", setCode: "ST-25" },
  { series: "622026", setCode: "ST-26" },
  { series: "622027", setCode: "ST-27" },
  { series: "622028", setCode: "ST-28" },
  { series: "622029", setCode: "ST-29" },
  { series: "622901", setCode: "Promotional Card" },
  { series: "622801", setCode: "Limited Edition Card" },
];
...
const normalizeSeriesSetCode = (value: string) => {
  const trimmed = value.trim();
  if (!trimmed) return "";
  const normalizedKey = normalizeKey(trimmed);
  const compact = normalizedKey.replace(/[^A-Z0-9]/g, "");
  if (normalizedKey === "OP14-EB04" || compact === "OP14EB04") {
    return "OP14-EB04";
  }
  return compact;
};
...
const missingUsBasePath = "scripts/missing-fr-base-jp.json";
...
const missingImagesPath = "scripts/missing-images-fr.json";
...
const uploadCardImage = async (
  ...
  const imageKey = options.versionImages ? `${baseKey}-${Date.now()}` : baseKey;
...
const parseArgs = (): ScriptOptions => {
  ...
  const options: ScriptOptions = {
    ...
    versionImages: args.includes("--version-images"),
...
const parseCardModal = (
  ...
  const normalizedCategory = normalizeKey(categoryRaw);
  const category =
    CATEGORY_MAP[normalizedCategory] || categoryRaw || "Character";
  const rarity = rarityRaw
    ? RARITY_MAP[rarityRaw.toUpperCase()] || rarityRaw
    : null;

  const costLabel = $modal.find(".cost h3").text().trim();
  const costValueRaw = normalizeLabelValue(
    $modal.find(".cost").text(),
    costLabel
  );
  const costNumber = parseNumericValue(costValueRaw);
  const normalizedCostLabel = normalizeKey(costLabel);
  const life =
    (normalizedCostLabel.includes("VIE") ||
      normalizedCostLabel.includes("LIFE")) &&
    costNumber
      ? `${costNumber} Life`
      : null;
  const cost =
    normalizedCostLabel.includes("COUT") || normalizedCostLabel.includes("COST")
      ? costNumber
        ? `${costNumber} Cost`
        : null
      : null;
  ...
  const powerValueRaw = normalizeLabelValue(
    $modal.find(".power").text(),
    powerLabel
  );
  ...
  const counterValueRaw = normalizeLabelValue(
    $modal.find(".counter").text(),
    counterLabel
  );
  ...
  const normalizedAttribute = normalizeKey(attributeAlt);
  const attribute =
    ATTRIBUTE_MAP[normalizedAttribute] ||
    ATTRIBUTE_MAP[normalizeKey(attributeAlt).replace(/\s+/g, "")] ||
    null;

  const colorLabel = $modal.find(".color h3").text().trim();
  const colorValueRaw = normalizeLabelValue(
  ...
  const colors = colorValueRaw
    ...
    .map((color) => {
      ...
      return COLOR_MAP[normalized] || normalized.toLowerCase();
    })
    .filter(Boolean);

  const featureLabel = $modal.find(".feature h3").text().trim();
  const featureValueRaw = normalizeLabelValue(
    $modal.find(".feature").text(),
    featureLabel
  );
  ...
  const textValueRaw = normalizeLabelValue(
  ...
  const triggerValueRaw = normalizeLabelValue(
    $modal.find(".trigger").text(),
    triggerLabel
  );
  const trigger = triggerValueRaw
    ? triggerValueRaw.replace(/\s+/g, " ").trim()
    : null;
  ...
  const getInfoRaw = normalizeLabelValue(
    $modal.find(".getInfo").text(),
    getInfoLabel
  );
  const normalizedInfoLabel = normalizeKey(getInfoLabel);
  const originTitle =
    normalizedInfoLabel.includes("EXTENSION") && getInfoRaw
      ? getInfoRaw.replace(/\s+/g, " ").trim()
      : null;
  if (originTitle) {
...
/** Descubre TODAS las series desde el <select> del sitio (auto por región). */
const fetchAllSeries = async (): Promise<SeriesEntry[]> => {
  const url = new URL(CARDLIST_PATH, BASE_URL);
  const response = await axios.get(url.toString(), {
    headers: { "User-Agent": HTTP_UA },
    maxRedirects: 5,
  ...
  $("option").each((_, el) => {
    ...
    const m = label.match(/\[([^\]]+)\]/);
    const setCode = m ? m[1].trim() : label;
    if (!map.has(series)) map.set(series, { series, setCode });
  });
...
const resolveSeriesList = async (
  ...
  if (options.setFilter?.length) {
    const normalizedOrder = options.setFilter.map((value) =>
      normalizeSeriesSetCode(value)
    );
    const filterSet = new Set(normalizedOrder);
    list = list
      .filter((entry) => filterSet.has(normalizeSeriesSetCode(entry.setCode)))
      .sort(
        (a, b) =>
          normalizedOrder.indexOf(normalizeSeriesSetCode(a.setCode)) -
          normalizedOrder.indexOf(normalizeSeriesSetCode(b.setCode))
      );
  }
...
const resolveSetId = async (setCode: string, region?: string | null) => {
  const normalized = normalizeSeriesSetCode(setCode);
  ...
  try {
    ...
    if (status === 404 || status === 410) {
      ...
      if (
        ...
        console.log(
          `[skip][network-error] ${card.id} ${card.imageUrl} (code ${code})`
        );
...
const main = async () => {
  ...
  // Resolver región → dominio, source y idioma.
  const regionKey = options.region.toUpperCase();
  const cfg = REGION_CONFIG[regionKey];
  if (!cfg) {
    console.error(
      `Región no soportada: "${options.region}". Usa una de: ${Object.keys(
        REGION_CONFIG
      ).join(", ")}`
    );
    return;
  ...
  SOURCE_TAG = cfg.region;
  options.region = cfg.region;
  options.language = cfg.language;
  console.log(`[region] ${cfg.region} → ${BASE_URL} (source=${SOURCE_TAG})`);
  ...
  for (const entry of seriesList) {
    ...
    const cards = (await fetchSeriesCards(entry.series, options)).map(
      (card) => {
        const normalizedSetCode = normalizeSeriesSetCode(entry.setCode);
        const resolvedSetCode = options.overrideSetCode
          ? normalizedSetCode || card.setCode
          : card.setCode;
        return {
          ...card,
    ...
    const resolveSetIdCached = async (code: string) => {
      const normalized = normalizeSeriesSetCode(code);
      if (!normalized) return null;
  ...
  if (options.cleanupDuplicates) {
    await cleanupDuplicateSources(SOURCE_TAG, options.region, options.dryRun);
  }
};
...
main()
  ...
  .finally(async () => {
    ...
    if (missingImages.length) {
      console.log(
        `\n[summary] Missing images (${missingImages.length}):`,
        missingImages.slice(0, 50)
      );
      if (missingImages.length > 50) {
        console.log(
          `[summary] ${
            missingImages.length - 50
          } more missing images not shown.`
        );
      }
      await fs.writeFile(
        missingImagesPath,
    ...
    if (networkErrors.length && !missingImages.length) {
      await fs.writeFile(
        missingImagesPath,
        JSON.stringify({ missingImages, networkErrors }, null, 2)
      );
